...
- [AUTH.md](/auth) — Auth flows, session management, role system, security
- [DATABASE.md](/database) — profiles, private_profiles, user_addresses, notifications tables
- [ROUTES.md](/routes) — (auth) and (account) route groups
- [selling.md](./selling.md) — Seller onboarding → Stripe Connect → first listing
- [PAYMENTS.md](/payments) — Stripe Connect onboarding for sellers
