...
export default async function OrderDetailPage({ params }: OrderDetailPageProps) {
  ...
  return withAccountPageShell(params, async ({ locale, supabase, user }) => {
    const { data: orderData, error: orderError } = await supabase
      .from("orders")
      .select(
        "id,user_id,total_amount,status,shipping_address,created_at,stripe_payment_intent_id"
      )
      .eq("id", id)
      .eq("user_id", user.id)
      .single()
    ...
    const productsData = (productsRaw || []) as Array<{
      id: string
      title: string
      slug: string | null
      images: string[] | null
    ...
    const sellerProfilesData = (sellerProfilesRaw || []) as Array<{
      id: string
    ...
    const sellerProfilesMap = new Map(
      sellerProfilesData.map((profile) => [profile.id, profile])
    )
    ...
    return {
      title: locale === "bg" ? `Поръчка #${id.slice(0, 8)}` : `Order #${id.slice(0, 8)}`,
      content: (
        <OrderDetailContent
          locale={locale}
          order={order}
