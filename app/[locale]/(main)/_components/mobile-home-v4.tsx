...
import { BannerL2Explorer } from "./mobile/banner-l2-explorer"
...
import {
  useHomeV4DiscoveryFeed,
  type HomeV4DiscoveryScope,
} from "@/hooks/use-home-v4-discovery-feed"

// =============================================================================
// Types
// =============================================================================

interface MobileHomeV4Props {
  locale: string
...
const PRIMARY_TAB_BASE_CLASS =
  "relative inline-flex min-h-(--control-default) items-center gap-1.5 whitespace-nowrap px-3 text-xs tap-transparent transition-colors duration-fast ease-smooth focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus-ring focus-visible:ring-inset"
...
const DISCOVERY_SCOPES: readonly HomeV4DiscoveryScope[] = [
  "forYou",
  "newest",
  "promoted",
  "nearby",
...
export function MobileHomeV4({
  ...
  const handleScopeSelect = useCallback((nextScope: HomeV4DiscoveryScope) => {
  ...
  return (
    <PageShell variant="default" className="pb-4">
      ...
      <div className="mx-auto w-full max-w-(--breakpoint-md) pb-tabbar-safe">
        ...
        {/* Banner: L2 explorer when subcategory has children, else text banner */}
        {activeSubcategorySlug && activeL2Categories.length > 0 ? (
          <BannerL2Explorer
            categories={activeL2Categories}
            activeSlug={activeL2Slug}
            subcategoryLabel={contextTitle}
            onSelect={handleL2Select}
            viewAllHref={fullBrowseHref}
          />
        ) : (
          <section data-testid="home-v4-context-banner" className="px-3 pt-2">
            <div className="flex min-h-(--control-default) items-center justify-between gap-2 rounded-xl border border-border-subtle bg-surface-subtle px-3">
              <div className="min-w-0 py-1.5">
                ...
                <h2 className="truncate text-sm font-semibold text-foreground">{contextTitle}</h2>
              </div>
              <Link href={fullBrowseHref} className={ACTION_CHIP_CLASS}>
