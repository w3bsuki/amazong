...
export default async function SharedWishlistTokenPage({ params }: SharedWishlistPageProps) {
  ...
  const { data: wishlistItems, error } = await supabase.rpc("get_shared_wishlist", { p_share_token: token })
    ...
    <PageShell variant="muted">
      <div className="container py-8">
        <div className="mb-8 text-center">
          ...
          {wishlistDescription && (
            <p className="mt-2 text-foreground max-w-2xl mx-auto">{wishlistDescription}</p>
          )}
          <p className="text-sm text-muted-foreground mt-4">{t("itemCount", { count: wishlistItems.length })}</p>
