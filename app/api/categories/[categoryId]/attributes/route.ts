...
const CategoryIdParamSchema = z.string().uuid()

async function getCategoryAttributesCached(categoryId: string) {
  'use cache'
...
export async function GET(
  ...
  try {
    ...
    const categoryIdResult = CategoryIdParamSchema.safeParse(rawCategoryId)

    if (!categoryIdResult.success) {
      return noStoreJson({ error: "Category ID is required" }, { status: 400 })
    }

    const categoryId = categoryIdResult.data

    const result = await getCategoryAttributesCached(categoryId)
    if (!result.ok) {
