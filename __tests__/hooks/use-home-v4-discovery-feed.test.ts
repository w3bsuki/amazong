...
import { useHomeV4DiscoveryFeed } from "@/hooks/use-home-v4-discovery-feed"
...
describe("hooks/use-home-v4-discovery-feed", () => {
  beforeEach(() => {
  ...
  it("uses initial pool products before network fetch", async () => {
    ...
    const { result } = renderHook(() =>
      useHomeV4DiscoveryFeed({ initialPools: pools, initialScope: "forYou", limit: 24 })
    )
  ...
  it("maps promoted scope to type=promoted", async () => {
    ...
    const { result } = renderHook(() =>
      useHomeV4DiscoveryFeed({ initialPools: createEmptyPools(), initialScope: "forYou", limit: 24 })
    )
