...
export function ConversationList({
  ...
  const filteredConversations = useMemo(() => {
      ...
      filtered = filtered.filter((conv) => {
        switch (filter) {
          case "unread":
            ...
            return (conv.buyer_unread_count || 0) > 0 || (conv.seller_unread_count || 0) > 0
          case "buying":
    ...
    if (searchQuery.trim()) {
      ...
      filtered = filtered.filter((conv) => {
        const displayName = conv.seller?.business_name || conv.seller?.profile?.full_name || conv.buyer?.full_name || ""
    ...
    return (
      <div className={cn("flex flex-col items-center justify-center h-full p-8", className)}>
        <div className="flex size-16 items-center justify-center rounded-full bg-muted mb-4">
          <ChatCircle size={32} weight="regular" className="text-muted-foreground" />
        </div>
        <p className="text-sm font-medium text-foreground mb-1">
          {searchQuery ? t("noSearchResults") || "No results found" : t("noConversations")}
        </p>
...
function ConversationItem({ conversation, isSelected, onClick, locale }: ConversationItemProps) {
  const t = useTranslations("Messages")
  ...
  const formatTime = (date: string) => {
    ...
    if (diffInHours < 24) return `${diffInHours}${locale === "bg" ? "ч" : "h"}`
  ...
  const timeDisplay = conversation.last_message_at ? formatTime(conversation.last_message_at) : ""
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-1.5">
          ...
          {isOwnMessage && lastMessage && (
            lastMessage.is_read ? (
              <Checks size={14} weight="bold" className="shrink-0 text-primary" />
            ) : (
              <Check size={14} weight="regular" className="shrink-0 text-muted-foreground" />
            )
          )}
          {/* Message preview */}
          <p className={cn(
          ...
          {hasUnread && (
            <div className="shrink-0 size-3 rounded-full bg-primary" />
          )}
        </div>
      </div>
    </button>
