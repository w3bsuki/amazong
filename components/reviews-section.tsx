...
export function ReviewsSection({ rating, reviewCount }: { rating: number, reviewCount: number }) {
    ...
    return (
        <div className="mt-6 sm:mt-8 grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-6 sm:gap-8 lg:gap-12">
            ...
            <div>
                <h2 className="text-xl sm:text-2xl font-bold text-[#0F1111] mb-2">{t('customerReviews')}</h2>
                <div className="flex items-center gap-2 mb-2">
                    <div className="flex text-[#f08804]">
                        {[...Array(5)].map((_, i) => (
                            <Star
                                key={i}
                                className={`h-4 w-4 sm:h-5 sm:w-5 ${i < Math.floor(rating) ? "fill-current" : "text-transparent stroke-current stroke-1 text-[#f08804]"}`}
                            />
                        ))}
                    </div>
                    <span className="text-base sm:text-lg font-medium text-[#0F1111]">{rating} {t('outOf5')}</span>
                </div>
                <div className="text-[#565959] text-sm mb-4 sm:mb-6">{t('globalRatings', { count: reviewCount })}</div>

                <div className="space-y-2 sm:space-y-3 mb-6 sm:mb-8">
                    {[5, 4, 3, 2, 1].map((star) => (
                        <div key={star} className="flex items-center gap-2 sm:gap-3 text-sm text-[#007185] hover:text-[#c7511f] hover:underline cursor-pointer group">
                            <span className="w-10 sm:w-12 text-[#0F1111] group-hover:text-[#c7511f] group-hover:underline text-xs sm:text-sm">{star} {t('star')}</span>
                            <Progress value={star === 5 ? 70 : star === 4 ? 20 : 5} className="h-4 sm:h-5 flex-1 rounded-md bg-[#f0f2f2] [&>div]:bg-[#f08804] border border-[#e3e6e6] shadow-inner" />
                            <span className="w-8 text-right text-xs sm:text-sm">{star === 5 ? "70%" : star === 4 ? "20%" : "5%"}</span>
                        </div>
                    ))}
                </div>
                ...
                <Button variant="outline" className="w-full min-h-11 rounded-md border-[#d5d9d9] hover:bg-[#f7fafa]">
                    {t('writeReview')}
                </Button>
            </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="flex gap-2">
                            <Button variant="outline" className="h-8 px-4 rounded-md border-[#d5d9d9] text-sm hover:bg-[#f7fafa]">
                                {t('helpful')}
                            </Button>
