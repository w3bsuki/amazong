...
export function StepCategory({
  ...
  const [_isLoadingAttributes, setIsLoadingAttributes] = useState(false);
  ...
  // Check validity
  const isValid = !!categoryId && !!condition;
  ...
  const handleAttributeChange = (attrName: string, value: string) => {
    const existing = attributes.filter(a => a.name !== attrName);
    if (value) {
      form.setValue("attributes", [...existing, { name: attrName, value, isCustom: false }], { shouldValidate: true });
    } else {
  ...
  const addCustomAttribute = () => {
    if (newAttrName && newAttrValue) {
      ...
      handleAttributeChange(newAttrName, newAttrValue);
      setNewAttrName("");
      setNewAttrValue("");
    }
  };
