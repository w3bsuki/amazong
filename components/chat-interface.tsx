...
import { useMessages, type Message } from "@/lib/message-context"
      ...
      <div className={cn("flex flex-col items-center justify-center h-full bg-background", className)}>
        <div className="flex flex-col items-center gap-4 p-8 text-center max-w-sm">
          <div className="flex size-20 items-center justify-center rounded-full border-2 border-foreground/20">
            <svg className="size-10 text-foreground/60" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M8.625 12a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H8.25m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H12m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0h-.375M21 12c0 4.556-4.03 8.25-9 8.25a9.764 9.764 0 01-2.555-.337A5.972 5.972 0 015.41 20.97a5.969 5.969 0 01-.474-.065 4.48 4.48 0 00.978-2.025c.09-.457-.133-.901-.467-1.226C3.93 16.178 3 14.189 3 12c0-4.556 4.03-8.25 9-8.25s9 3.694 9 8.25z" />
            </svg>
          </div>
          <div>
            <h3 className="text-lg font-semibold text-foreground mb-1">{t("selectConversation")}</h3>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <h2 className="font-semibold text-base text-foreground truncate">{displayName}</h2>
              </div>
              {currentConversation.product ? (
                ...
                <p className="text-xs text-muted-foreground">
                  {isClosed 
                    ? (locale === "bg" ? "Затворен чат" : "Chat closed")
        ...
        {isLoadingMessages ? (
          ...
          <div className="space-y-1">
            {messagesWithSeparators.map((item, index) => {
              if (item.type === "separator") {
                ...
                if (nextItem?.type === "message") {
                  const isNextOrderNotification = 
                    (nextItem.message.message_type === 'system' || nextItem.message.message_type === 'order_update') &&
                    (nextItem.message.content.includes('New Order') || nextItem.message.content.includes('Нова поръчка'))
              ...
              const isSystemMessage = message.message_type === 'system' || message.message_type === 'order_update'
              ...
              const _isFirstInGroup = !prevItem || 
              ...
              if (isSystemMessage) {
                ...
                const isOrderNotification = message.content.includes('New Order') || message.content.includes('Нова поръчка')
