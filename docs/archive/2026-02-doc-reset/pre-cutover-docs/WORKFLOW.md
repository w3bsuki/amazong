...
### Prompt Packet v2 (Required)

**Mode (allowed):** `implement` | `review` | `plan` | `audit`

**Risk (allowed):** `low-risk` | `high-risk`

**Default rules:**
- `review` and `audit` are non-mutating (no code changes)
- `implement` is mutating (code changes allowed) and MUST run gates
...
- `docs/HARNESS.md` — Execution harness contract (failures, retries, evidence)
