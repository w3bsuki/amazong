...
| Planning & priorities | `docs/PLANS.md` | Current phase, active priorities, decision framework |
...
| Core beliefs | `docs/design-docs/core-beliefs.md` | Agent-first operating principles |
| Design decisions | `docs/design-docs/index.md` | Index of design docs with verification status |
| Product sense | `docs/PRODUCT_SENSE.md` | User personas, product principles, decision heuristics |
| Product specs | `docs/product-specs/index.md` | Index of all feature specifications |
| Quality grades | `docs/QUALITY_SCORE.md` | Domain quality heat map, improvement priorities |
| Production status | `docs/PRODUCTION.md` | What's shipped, what's blocked, launch checklist |
| Harness / execution rigor | `docs/HARNESS.md` | Failure handling, retries, timeouts, evidence, escalation |
...
| Generated DB schema | `docs/generated/db-schema.md` | Auto-generated table inventory from migrations |
...
| Security | `docs/SECURITY.md` | Threat model, auth security, secrets management |
| Reliability | `docs/RELIABILITY.md` | Uptime targets, resilience patterns, failure modes |
| Internationalization | `docs/I18N.md` | Locales (en/bg), namespaces, ICU patterns |
| Routes | `docs/ROUTES.md` | All routes with auth levels |
| Exec plans (active) | `docs/exec-plans/active/` | Multi-step initiatives in progress |
| Exec plans (completed) | `docs/exec-plans/completed/` | Finished plans for context |
| Tech debt | `docs/exec-plans/tech-debt-tracker.md` | Known debt inventory |
| External references | `docs/references/` | Pre-fetched external lib docs for agent use |
...
- **Freshness check:** `node scripts/check-doc-freshness.mjs` — scans for stale docs (>30 days)
- **Schema generation:** `node scripts/generate-db-schema.mjs` — regenerates `docs/generated/db-schema.md` from migrations
- **Architecture tests:** `pnpm -s test:unit __tests__/architecture-boundaries.test.ts` — structural boundary enforcement
- **Exec plans:** Complex multi-step work goes in `docs/exec-plans/active/` (not TASKS.md)
- **Quality grades:** Update `docs/QUALITY_SCORE.md` after completing feature or infrastructure work
- **Tech debt:** Record new debt in `docs/exec-plans/tech-debt-tracker.md`
...
| `node scripts/check-doc-freshness.mjs` | Doc freshness scan |
| `node scripts/generate-db-schema.mjs` | Auto-generate DB schema doc |
