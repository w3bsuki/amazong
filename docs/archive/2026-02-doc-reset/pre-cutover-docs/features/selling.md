...
- Requirements: 12/14 complete (R2: 7/8 + R5: 5/6) — see REQUIREMENTS.md §R2, §R5
- Production: 🟡 Partial — listing analytics not started; refund processing admin-assisted

## Requirements Mapping

| Req ID | Description | Status |
|--------|-------------|--------|
| R2.1 | Auth-gated sell entry (`/sell`) | ✅ |
| R2.2 | Multi-step listing wizard (title, desc, category, attributes, images, price) | ✅ |
| R2.3 | Image upload (multi-image, drag-reorder, compression) | ✅ |
| R2.4 | Category + attribute selection | ✅ |
| R2.5 | Draft → publish flow | ✅ |
| R2.6 | Edit listing | ✅ |
| R2.7 | Delete / unpublish listing | ✅ |
| R2.8 | Listing analytics (business tier only) | ⬜ Not started |
| R5.1 | Seller orders list | ✅ |
| R5.2 | Seller order detail | ✅ |
| R5.3 | Mark as shipped | ✅ |
| R5.4 | Mark as delivered | ✅ |
| R5.5 | Process refund (admin-assisted) | 🟡 In progress |
| R5.6 | Inventory updates (DB triggers) | ✅ |
...
- `products.ts` — Product CRUD (create, update, delete, publish, unpublish)
- `profile.ts` — Seller profile updates
...
| `categories` | Hierarchical L0→L3 category tree |
| `category_attributes` | Dynamic form fields per category |
| `brands` | Brand info with logos |
| `order_items` | Line items linking orders → products → sellers, with tracking |

### Key DB Functions

- `handle_new_product_search()` — Updates full-text search vector on product insert/update
- `sync_product_attributes_jsonb()` — Syncs EAV attributes to JSONB column
- `normalize_attribute_key()` — Normalizes attribute keys for consistent querying
- `decrement_stock_on_order()` — Decrements variant stock when order is placed

## Known Gaps & V1.1+ Items

| Item | Status | Notes |
|------|--------|-------|
| R2.8: Listing analytics | ⬜ Not started | Business tier only; dashboard analytics page exists but listing-level metrics not wired |
| R5.5: Process refund | 🟡 In progress | Currently admin-assisted; self-service refund initiation deferred |
| Bulk listing operations | ⬜ Deferred | V1.1 — bulk edit/delete for high-volume sellers |
| AI Listing Assistant | ⬜ Deferred | V2 — AI autofill from photos (`/api/assistant/sell-autofill` endpoint exists) |
...
- `docs/DESIGN.md` §8.5 — Sell drawer / wizard UX pattern
