...
5. **Token-safe styling only** (SSOT: `docs/DESIGN.md`).

---
## Mobile-First Production Contract (2026-02)

1. Visual system:
   - Neutral surfaces + blue accent + destructive red only.
   - No gradients, decorative animation, or palette classes.
   - No Tailwind arbitrary values and no token alpha hacks.
2. Interaction ergonomics:
   - All frequent touch controls are at least 44px.
   - Product quick-view remains default tap behavior in browse surfaces.
   - Filter/search/category flows preserve browsing context.
3. Layout density:
   - One primary filter control row per screen.
   - Tighter whitespace for scanability; chips stay secondary.
   - Product cards use lower corner radii for larger visible media area.
4. Delivery order:
   - Mobile first lock, then desktop parity, then repo-wide style debt sweep.
