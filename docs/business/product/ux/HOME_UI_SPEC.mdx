...
1) Scope controls appear as two compact triggers:
   - City
   - Category
2) Both controls open **bottom drawers** (no dropdown menus).
3) Scope updates Home sections instantly and updates section copy.
4) Clear scope resets to default curated Home.
5) No full filter hub, no sort drawer, no segmented sort controls on Home.
