...
export type HomeV4DiscoveryScope = "forYou" | "newest" | "promoted" | "nearby" | "deals"

interface HomeV4InitialPools {
  forYou: UIProduct[]
  newest: UIProduct[]
  promoted: UIProduct[]
  nearby: UIProduct[]
...
interface UseHomeV4DiscoveryFeedOptions {
  initialPools: HomeV4InitialPools
  initialCategoryProducts?: Record<string, UIProduct[]>
  initialScope?: HomeV4DiscoveryScope
  limit?: number
}
...
export function useHomeV4DiscoveryFeed({
  initialPools,
  initialCategoryProducts = {},
  initialScope = "forYou",
  limit = 24,
}: UseHomeV4DiscoveryFeedOptions) {
  const [scope, setScope] = useState<HomeV4DiscoveryScope>(initialScope)
