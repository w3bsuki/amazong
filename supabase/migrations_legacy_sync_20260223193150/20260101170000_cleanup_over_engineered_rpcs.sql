...
-- SUMMARY
-- =====================================================
-- Removed: 6 over-engineered RPC functions
-- Removed: ~60 unused indexes
-- 
-- Still available for use:
-- - All trigger functions (essential for data integrity)
-- - cart_add_item, cart_set_quantity, cart_clear (atomic operations)
-- - mark_notification_read, mark_all_notifications_read, get_total_unread_messages
-- - get_user_conversations, get_conversation_messages, get_or_create_conversation
-- - mark_messages_read
-- - get_shared_wishlist, enable_wishlist_sharing, disable_wishlist_sharing
-- - cleanup_sold_wishlist_items
-- - increment_helpful_count
-- - block_user, unblock_user, is_blocked_bidirectional, get_blocked_users
-- - is_admin
-- - get_category_ancestor_ids (used by triggers internally)
--
-- Client code should use direct Supabase queries:
-- - supabase.from('table').select().eq().filter()
-- - Use { count: 'exact' } for counts
-- - Use .filter('column', 'cs', '{value}') for array contains
-- =====================================================
