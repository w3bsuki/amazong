...
-- =====================================================
-- PART 7: REVIEW RESPONSE RLS UPDATE
-- Ensure sellers can update their own product reviews (for responses)
-- =====================================================

-- Allow sellers to update seller_response on their product reviews
DO $$
BEGIN
  -- Check if policy exists before creating
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'reviews' AND policyname = 'reviews_seller_response'
  ) THEN
    CREATE POLICY reviews_seller_response ON public.reviews
      FOR UPDATE
      USING (
        EXISTS (
...
-- =====================================================
-- PART 8: FIVE STAR REVIEW TRACKING
