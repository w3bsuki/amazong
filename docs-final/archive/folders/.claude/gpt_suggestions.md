...
- `_components`, `components/shared`, `components/layout`.
...
4) "UI Slop Sweep":
   - Run `slop-hunter` on `components/shared` and `app/**/_components`.
...
- "Use the slop-hunter agent to scan `components/shared` for AI bloat."
