...
- [x] Review submission increments rating count but new review does not appear in list without refresh → fixed via cache tag revalidation alignment (2026-01-23).
...
- Review submission increments rating count (now 2 global ratings) but new review does not appear in list without refresh. (Fixed 2026-01-23 via cache tag alignment.)
...
## Treido Audit - 2026-01-23 (P8 UI/UX + QA pass)

### Automated gates (run locally)
- ✅ Typecheck: `pnpm -s exec tsc -p tsconfig.json --noEmit`
- ✅ E2E smoke: `REUSE_EXISTING_SERVER=true pnpm test:e2e:smoke` (18 passed, 1 skipped: seller flow requires `TEST_USER_EMAIL`/`TEST_USER_PASSWORD`)
- ✅ Styles gate: `pnpm -s styles:gate`

### Implemented / verified changes
- Guest gating: `/sell`, `/chat`, and Wishlist drawer now show a **non-blocking** sign-in prompt (no forced redirect to login).
- Onboarding: post-signup flow (intent = Sell) now includes **Personal vs Business** selection and uses that value through onboarding completion.
- CTA/routing: `/help` now has Customer Service metadata to avoid title flicker during redirect.
- Stability: localized `not-found` no longer throws when `params` is undefined (observed in dev server logs during Playwright warmup).

### Manual QA checklist (HUMAN REQUIRED)

#### Locale: `/en`
- [ ] Locales: `/en/*` routing works end-to-end (HUMAN)
- [ ] Auth: signup → confirm email → login → reset password (HUMAN: requires inbox)
- [ ] Browse/search: home, categories, search filters (HUMAN)
- [ ] Product page: images, price, seller info, wishlist, add-to-cart (HUMAN)
- [ ] Seller flow: sell entry → payouts gate → create listing with images (HUMAN; Stripe Connect step may block)
- [ ] Messaging: start chat, send message, upload image (HUMAN)
- [ ] Checkout/orders (if in scope): cart → checkout → order appears for buyer + seller (HUMAN; known blocked in prior audit)
- [ ] Reviews: submit + helpful vote + delete own review (HUMAN)

#### Locale: `/bg`
- [ ] Locales: `/bg/*` routing works end-to-end (HUMAN)
- [ ] Auth: signup → confirm email → login → reset password (HUMAN: requires inbox)
- [ ] Browse/search: home, categories, search filters (HUMAN)
- [ ] Product page: images, price, seller info, wishlist, add-to-cart (HUMAN)
- [ ] Seller flow: sell entry → payouts gate → create listing with images (HUMAN; Stripe Connect step may block)
