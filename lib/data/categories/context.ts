...
type CategoryRowWithParent = Category & { parent: Category[] | Category | null }

async function buildCategoryContextFromCurrent(params: {
  supabase: ReturnType<typeof createStaticClient>
  current: CategoryRowWithParent
}): Promise<CategoryContext> {
  const { supabase, current } = params
...
export async function getCategoryContext(slug: string): Promise<CategoryContext | null> {
  ...
  return buildCategoryContextFromCurrent({
    supabase,
    current: current as CategoryRowWithParent,
  })
}

/**
 * Fetch complete category context for sidebar navigation by category ID.
 * Includes current category, parent, siblings, children, and filterable attributes.
 *
 * @param categoryId - Category UUID
 * @returns Full category context or null if not found
 */
export async function getCategoryContextById(categoryId: string): Promise<CategoryContext | null> {
  "use cache"
  ...
  if (currentError || !current) {
    logger.error("[getCategoryContextById] Category not found", currentError)
    return null
  }

  cacheTag(`category:${current.slug}`)
