...
// =============================================================================
// Category Path - Efficient Breadcrumb Fetching (No Nested Joins!)
// =============================================================================
...
/**
 * Fetch the full category path from root to leaf using the efficient DB function.
 * This replaces 4-level nested joins with a single RPC call.
 * 
 * USE THIS instead of nested category joins in product queries!
 * 
 * @param categoryId - The leaf category ID
 * @returns Array of categories from root (L0) to leaf (deepest)
 */
export async function getCategoryPath(categoryId: string): Promise<CategoryPathNode[]> {
  'use cache'
  cacheTag('categories', `category-path-${categoryId}`)
  cacheLife('categories')

  const supabase = createStaticClient()
  if (!supabase) return []

  // UUID validation - avoid DB errors on bad input
  const UUID_REGEX = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i
  if (!UUID_REGEX.test(categoryId)) return []

  // Call the RPC function - use type assertion since get_category_path isn't in generated types yet
  // Run `pnpm supabase:types` to regenerate types after migration
  const { data, error } = await (supabase.rpc as (
    fn: string, 
    args: { p_category_id: string }
  ) => ReturnType<typeof supabase.rpc>)('get_category_path', { p_category_id: categoryId })

  if (error) {
    console.error(`[getCategoryPath:${categoryId}]`, error.message)
    return []
  }

  // Transform DB rows to UI format, stripping [HIDDEN] prefixes
  const rows = (data as unknown) as Array<{ id: string; slug: string; name: string; name_bg: string | null; icon: string | null }> | null
  return (rows || []).map((row) => ({
    id: row.id,
    slug: row.slug,
    name: (row.name || '').replace(/^\s*\[HIDDEN\]\s*/i, '').trim(),
    nameBg: (row.name_bg || '').replace(/^\s*\[HIDDEN\]\s*/i, '').trim() || null,
    icon: row.icon,
  }))
}

/**
 * Batch fetch category paths for multiple products.
 * More efficient than individual calls when rendering product lists.
 * 
 * @param categoryIds - Array of unique category IDs
 * @returns Map of categoryId -> path
 */
export async function getCategoryPathsBatch(
  categoryIds: string[]
): Promise<Map<string, CategoryPathNode[]>> {
  'use cache'
  // Use a hash of IDs for cache tag (sorted for consistency)
  const sortedIds = [...new Set(categoryIds)].sort()
  cacheTag('categories', `category-paths-batch-${sortedIds.length}`)
  cacheLife('categories')

  const results = new Map<string, CategoryPathNode[]>()
