/**
 * Deterministic checks for spec-supabase audit output
 * Run: node .codex/evals/skills/spec-supabase/checks.mjs <output-file>
 */
...
export function validateSupabaseAudit(outputText) {
  const results = {
  ...
  // Check heading
  if (outputText.startsWith('## SUPABASE')) {
    results.heading_correct = true;
  } else {
    results.errors.push('Output must start with "## SUPABASE"');
  }
  ...
  // Check ID sequencing
  const idMatches = outputText.match(/SUPABASE-\d{3}/g) || [];
  ...
  if (findingsSection.includes('No findings')) {
    ...
    const hasEvidence = /\w+\.(tsx?|jsx?|ts|js|sql):\d+/.test(findingsSection);
  ...
  // Check read-only
  const forbiddenPatterns = ['replace_string_in_file', 'create_file', 'edit TASKS.md', 'apply_migration'];
  ...
  // Check no secrets (service keys, env values)
  const secretPatterns = [
    /SUPABASE_SERVICE_ROLE_KEY\s*[=:]\s*[^\s]+/,
    /eyJ[A-Za-z0-9\-_]+\.[A-Za-z0-9\-_]+/, // JWT pattern
    /service_role[=:]\s*[^\s]+/i,
  ];
  for (const pattern of secretPatterns) {
    if (pattern.test(outputText)) {
      results.no_secrets = false;
      results.errors.push('Output contains potential secrets/keys');
    }
  }
...
if (process.argv[2]) {
  ...
  const results = validateSupabaseAudit(content);
